...
mysql_query("INSERT INTO `googleplused` (user_id, site_id) VALUES('{$posts['user']}', '{$posts['id']}')");
echo "Done!";
}else{
$id = $posts['id'];
$user = $posts['user'];
$mysite = mysql_fetch_object(mysql_query("SELECT cpc, user FROM `googleplus` WHERE `id`='{$id}'"));
$check = mysql_num_rows(mysql_query("SELECT id FROM `googleplused` WHERE `user_id`='{$user}' AND `site_id`='{$id}'"));
if($id != "" && $user != "" && $check == 0){
...
mysql_query("UPDATE `googleplus` SET `exchanges`=`exchanges`+'1' WHERE `id`='{$id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`-'{$mysite->cpc}' WHERE `id`='{$mysite->user}'");
mysql_query("INSERT INTO `googleplused` (user_id, site_id) VALUES('{$user}','{$id}')");
