...
if(isset($posts['site'])){
$site2 = mysql_fetch_object(mysql_query("SELECT * FROM `surf` WHERE `id`='{$posts['site']}'"));
$check = mysql_num_rows(mysql_query("SELECT * FROM `surfed` WHERE `user_id`='{$posts['userid']}' AND `site_id`='{$posts['site']}'"));
...
mysql_query("UPDATE `surf` SET `exchanges`=`exchanges`+'1' WHERE `id`='{$site2->id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`-'{$site2->cpc}' WHERE `id`='{$site2->user}'");
mysql_query("INSERT INTO `surfed` (user_id, site_id) VALUES('{$posts['userid']}','{$posts['site']}')");
